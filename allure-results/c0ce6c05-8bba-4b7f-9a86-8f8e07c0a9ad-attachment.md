...
- Name: loginpagefixture.spec.ts >> Login Page Tests >> login with valid credentials test
- Location: tests/loginpagefixture.spec.ts:18:9
        ...
        - generic [ref=f1e87]:
          ...
          - generic [ref=f1e91]:
            - generic [ref=f1e92]:
              ...
              - textbox "E-Mail Address" [ref=f1e94]: pwbatchtest@open.com
            - generic [ref=f1e95]:
              - generic [ref=f1e96]: Password
              - textbox "Password" [ref=f1e97]: pw1234
...
```ts
  ...
  5  | });
  6  | 
  7  | test.describe("Login Page Tests", () => {
  8  |     test('login page title test', async ({ loginPage }) => {
  9  |         const title = await loginPage.getLoginPageTitle();
  10 |         expect(title).toBe("Account Login");
  11 |     });
  12 | 
  13 |     test('forgot password link visibility test', async ({ loginPage }) => {
  14 |         const isVisible = await loginPage.isForgotPasswordLinkVisible();
  15 |         expect(isVisible).toBe(true);
  16 |     });
  17 | 
  18 |     test('login with valid credentials test', async ({ loginPage, homePage }) => {
  19 |         await loginPage.login(process.env.USERNAME!, process.env.PASSWORD!);
  20 |         const title = await homePage.gethomepageTitle();
> 21 |         expect(title).toBe("My Account");
     |                       ^ Error: expect(received).toBe(expected) // Object.is equality
  22 |     });
  23 |     
  24 | })
